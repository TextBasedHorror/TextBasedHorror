...
horror_info[22] = new horror(false, true, false, 15, 23, false, false);
...
yes_options[24] = "GO FOR THE DOOR";
no_options[24] = "GRAB A BOOK";
...
horror_info[32] = new horror(false, false, false, 33, 34, false, false);
...
horror_info[40] = new horror(false, false, false, 41, 42, false, false);
...
horror_info[45] = new horror(false, false, false, 46, 47, false, false);
...
horror_info[50] = new horror(false, false, true, 51, 35, false, false);
...
horror_info[56] = new horror(false, false, false, 57, 58, false, false);
...
horror_info[61] = new horror(false, false, false, 62, 63, false, false);
...
function random_type(chara){
	...
	if (chara === '.' || chara === '?' || chara === '!') {
	    rando_value = (Math.random() * 20) + (Math.sin(sine_wave) * 80) + 80;
	}
	else {
		rando_value = (Math.random() * 20) + (Math.sin(sine_wave) * 30) + 30;
	}
	return rando_value;
}
...
function dramatic_parse(sentence, callback) {
    ...
    var i = 0;
	var mod = 0;
    var calls_left = sentence_list.length;
	*/
	//var i = 0;
	type_speed = 60;
    //$('<p></p>').appendTo($("#instructions")).append("WORDS");
    ...
    for (iterate = 0; iterate < total_calls; iterate++) {
	    ...
	    if (keep_iter <= total_calls) {
				...
				type_speed = type_speed + 0;
				clearTimeout(text_timer);
				$("#instructions").append(sentence.slice(keep_iter));
				return;
			}*/
	    }
		//i++;
	...
	/*
    while (calls_left != 0){
        var validate_end = sentence_list[i].length;
        mod = spooky_type(sentence_list[i],0);
        if (mod != 0) {
            calls_left += mod;
            i++;
			mod = 0;
...
function dead_dead(death_cursor) {
    ...
    dramatic_parse(failArray[death_cursor],function(){
		dramatic_parse("Do you want to play again?", function(){
